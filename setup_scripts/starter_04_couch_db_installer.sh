...
sudo rm -rf $COUCH_DB_FOLDER_TARGET_PATH"/"$COUCH_DB_DOWNLOAD_FOLDER_NAME"/"


#Create, Register and Install a CouchDB Starter service after system reboot
echo "Create a systemd service for CouchDB"
sudo cp -u $COUCH_DB_DAEMON_SYSTEMD_SERVICE_FILE_PATH /etc/systemd/system/couchdb.service
